...
export async function PUT(request: Request, context: RouteContext) {
  ...
  if (!/^\d+$/.test(id) || Number(id) < 1) {
    return NextResponse.json({ error: "固定費が見つかりません。" }, { status: 404 });
  }
  ...
  const response = await fetch(`${workerUrl}/fixed-costs/${id}`, {
    method: "PUT",
    headers: {
  ...
  if (!response) {
    return NextResponse.json({ error: "固定費を保存できませんでした。" }, { status: 502 });
  }

  const responseBody = await response.json().catch(() => ({
    error: "固定費を保存できませんでした。",
  }));
