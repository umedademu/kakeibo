...
export async function PUT(request: Request, context: RouteContext) {
  ...
  if (!/^\d+$/.test(id) || Number(id) < 1) {
    return NextResponse.json({ error: "借金が見つかりません。" }, { status: 404 });
  }
  ...
  const response = await fetch(`${workerUrl}/debts/${id}`, {
    method: "PUT",
    headers: {
  ...
  if (!response) {
    return NextResponse.json({ error: "借金を保存できませんでした。" }, { status: 502 });
  }
